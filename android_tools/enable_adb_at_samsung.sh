...
# ---------------------------------------------------------------------------------------------
    ...
    LOG_METHOD_13="${DIR}"/LOGS/TyR/METHODS/13/LOGS/
    if [ ! -d "${LOG_METHOD_13}" ]; then
        mkdir -p "${LOG_METHOD_13}";
        cd  "${LOG_METHOD_13}" || return;
        mkdir "${input_dir}" && cd "${input_dir}" || exit

    elif [ -d "${LOG_METHOD_13}" ]; then
        cd "${LOG_METHOD_13}" || return
        mkdir "${input_dir}" && cd "${input_dir}" || exit
        ...
        mdm_cnt_logic=$(mdm_cnt_logic)
            ...
            if [[ ${modem_count} -ge 1 ]]; then
               ...
               echo "Enabling ADB Do Not Unplug"
                double_space
                    echo "AT+SYSSCOPE=1,0" | socat -t 2 -,escape=0x0f `ls  /dev/cu.usbmodem*`,crnl 1>/dev/null
                    echo "AT+USBMODEM" | socat -t 2 -,escape=0x0f `ls  /dev/cu.usbmodem*`,crnl 1>/dev/null
                    sleep 5
                    echo "AT+CTSA=2,170,1209" | socat -t 2 -,escape=0x0f `ls  /dev/cu.usbmodem*`,crnl 1>/dev/null
                    echo "AT+CTSA=2,1140,1558" | socat -t 2 -,escape=0x0f `ls  /dev/cu.usbmodem*`,crnl 1>/dev/null
                    echo "AT+CTSA=2,880,1420" | socat -t 2 -,escape=0x0f `ls  /dev/cu.usbmodem*`,crnl 1>/dev/null
                    echo "AT+CTSA=2,928,1365" | socat -t 2 -,escape=0x0f `ls  /dev/cu.usbmodem*`,crnl 1>/dev/null
                    echo "AT+CTSA=2,1200,1786" | socat -t 2 -,escape=0x0f `ls  /dev/cu.usbmodem*`,crnl 1>/dev/null
