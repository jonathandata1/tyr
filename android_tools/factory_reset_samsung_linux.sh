...
# ---------------------------------------------------------------------------------------------
    ...
    LOG_METHOD_9="${DIR}"/LOGS/TyR/METHODS/9/LOGS/
    if [ ! -d "${LOG_METHOD_9}" ]; then
        mkdir -p "${LOG_METHOD_9}";
        cd  "${LOG_METHOD_9}" || return;
        mkdir "${input_dir}" && cd "${input_dir}" || exit

    elif [ -d "${LOG_METHOD_9}" ]; then
        cd "${LOG_METHOD_9}" || return
        mkdir "${input_dir}" && cd "${input_dir}" || exit
...
# ---------------------------------------------------------------------------------------------
    lg_reset(){
     ...
     double_space
     echo "AT+FACTORST=0,0" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl
    }
