...
# NOTE: This is written for Linux Only
#
...
#    ATZ &>/dev/null
    echo "ATZ" | socat -t 2 - escape=0x0f `ls /dev/ttyUSB*`, crnl &>/dev/null

    echo "AT+CGMI" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_manufacturer_var_AT_2.log
    ...
    #################### MODEL ####################################
    echo "ATZ" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/dev/null
    echo "AT+GMM" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_model_AT.log
   ...
   #################### IMEI ####################################
    ...
    echo "ATZ" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/dev/null
    echo "AT+GSN" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_imei_AT_1.log
    echo "AT+IMEINUM" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_imei_AT_2.log
    ...
    echo "AT+VERSNAME=3,2,3" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_ver_AT.log
    aos_sam_ver_AT=$(cat /tmp/aos_sam_ver_AT.log | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl | awk -F ':3,' '{print $NF}' | sed '/AT+VERSNAME=3,2,3/d;/OK/d' | uniq)
    ...
    echo "AT+VERSNAME=3,2,2" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_os_AT.log
    aos_sam_os_AT=$(cat /tmp/aos_sam_os_AT.log | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl | awk -F ':3,' '{print $NF}' | sed '/AT+VERSNAME=3,2,2/d;/OK/d' | uniq)
    ...
    echo "ATZ" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/dev/null
    sleep 2
    echo "AT+GMR" | socat -t 3 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_firmware_ver_AT_1.log
    ...
    echo "AT+SERIALNO" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_serial_no_AT.log
    ...
    echo "AT+SIZECHECK=" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_memory_size_AT.log
    ...
    echo "AT+SYSSCOPE=1,0" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_rooted_status.log
    ...
    echo "AT+SVCIFPGM=1,4" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_sam_network_lock_status.log
    ...
    echo "AT+DEVCONINFO" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_carrier_code_AT.log
    echo "AT+DEVCONINFO" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_carrier_code_AT.log
    echo "AT+DEVCONINFO" | socat -t 2 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_carrier_code_AT.log
    ...
    echo "AT+REACTIVE=1,0,0" | socat -t 3 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/frp_lock_status.log
    ...
    echo "AT+CBC" | socat -t 3 -,escape=0x0f `ls /dev/ttyUSB*`,crnl &>/tmp/aos_battery_percentage_AT.log
